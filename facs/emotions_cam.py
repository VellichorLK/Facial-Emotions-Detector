...
def get_landmarks_eyes(image):
        ...
        xshape = []
        yshape = []
        for k in range(36,68): #Store X and Y coordinates in two lists
            xshape.append(float(shape.part(k).x))
            yshape.append(float(shape.part(k).y))
        xmean = np.mean(xshape) #Find both coordinates of centre of gravity
        ymean = np.mean(yshape)
...
def get_landmarks_mouth(image):
        ...
        xshape = []
        yshape = []
        for k in range(48,68): #Store X and Y coordinates in two lists
            xshape.append(float(shape.part(k).x))
            yshape.append(float(shape.part(k).y))
        xmean = np.mean(xshape) #Find both coordinates of centre of gravity
        ymean = np.mean(yshape)
        ...
        for x, y, w, z in zip(xcentral, ycentral, xshape, yshape): # paara hacer pares 
            ...
            landmarks_vectorised.append((math.atan2(y, x)*360)/(2*math.pi))
        return landmarks_vectorised
def get_landmarks_mouth1(image):
        ...
        landmarks_vectorised = []
        for x, y, w, z in zip(xcentral[48:68], ycentral[48:68], xshape[48:68], yshape[48:68]): # paara hacer pares 
            landmarks_vectorised.append(w)
            landmarks_vectorised.append(z)
            meannp = np.asarray((ymean,xmean)) #convert input to an array
            coornp = np.asarray((z,w))
...
def get_landmarks_brows(image):
        ...
        landmarks_vectorised = []
        for x, y, w, z in zip(xcentral[17:27], ycentral[17:27], xshape[17:27], yshape[17:27]): # paara hacer pares 
            landmarks_vectorised.append(w)
            landmarks_vectorised.append(z)
            meannp = np.asarray((ymean,xmean)) #convert input to an array
            coornp = np.asarray((z,w))
...
def get_landmarks_nose(image):
        ...
        landmarks_vectorised = []
        for x, y, w, z in zip(xcentral[27:36], ycentral[27:36], xshape[27:36], yshape[27:36]): # paara hacer pares 
            landmarks_vectorised.append(w)
            landmarks_vectorised.append(z)
            meannp = np.asarray((ymean,xmean)) #convert input to an array
            coornp = np.asarray((z,w))
...
def get_landmarks_jaws(image):
        ...
        landmarks_vectorised = []
        for x, y, w, z in zip(xcentral[0:17], ycentral[0:17], xshape[0:17], yshape[0:17]): # paara hacer pares 
            landmarks_vectorised.append(w)
            landmarks_vectorised.append(z)
            meannp = np.asarray((ymean,xmean)) #convert input to an array
            coornp = np.asarray((z,w))
...
def get_landmarks_brows2(image):
        frame = resized_frame(image)
        rect = resize_detection(image)
        if rect is None:
            return None
        ...
        xshape = []
        yshape = []
        for k in range(17,27): #Store X and Y coordinates in two lists
            xshape.append(float(shape.part(k).x))
            yshape.append(float(shape.part(k).y))
        xmean = np.mean(xshape) #Find both coordinates of centre of gravity
        ymean = np.mean(yshape)
...
def get_landmarks_browseyes(image):
        frame = resized_frame(image)
        rect = resize_detection(image)
        if rect is None:
            return None
        ...
        xshape = []
        yshape = []
        for k in range(17,46): #Store X and Y coordinates in two lists
            xshape.append(float(shape.part(k).x))
            yshape.append(float(shape.part(k).y))
        xmean = np.mean(xshape) #Find both coordinates of centre of gravity
        ymean = np.mean(yshape)
